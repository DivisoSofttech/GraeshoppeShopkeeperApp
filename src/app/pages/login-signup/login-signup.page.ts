...
export class LoginSignupPage implements OnInit {
  ...
  createStore(userName) {
    ...
    this.commandResource.createStoreUsingPOST(store).subscribe(res => {
      this.util.createToast('Successfully Registered');
    }, err => {
      this.util.createToast('Registration Incomplete');
    });
  }
